...
class FSRS:
    ...
    def init_ds(self, s: SchedulingCards) -> None:
        s.again.difficulty = self.init_difficulty(1)
        s.again.stability = self.init_stability(1)
        s.hard.difficulty = self.init_difficulty(2)
        s.hard.stability = self.init_stability(2)
        s.good.difficulty = self.init_difficulty(3)
        s.good.stability = self.init_stability(3)
        s.easy.difficulty = self.init_difficulty(4)
        s.easy.stability = self.init_stability(4)

    def next_ds(self, s: SchedulingCards, last_d: float, last_s: float, retrievability: float):
        s.again.difficulty = self.next_difficulty(last_d, 1)
        s.again.stability = self.next_forget_stability(last_d, last_s, retrievability)
        s.hard.difficulty = self.next_difficulty(last_d, 2)
        s.hard.stability = self.next_recall_stability(last_d, last_s, retrievability, 2)
        s.good.difficulty = self.next_difficulty(last_d, 3)
        s.good.stability = self.next_recall_stability(last_d, last_s, retrievability, 3)
        s.easy.difficulty = self.next_difficulty(last_d, 4)
        s.easy.stability = self.next_recall_stability(last_d, last_s, retrievability, 4)
